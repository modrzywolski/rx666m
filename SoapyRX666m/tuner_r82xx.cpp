 ...
 */
void LowLevel::shadow_store(struct r82xx_priv *priv, uint8_t reg, const uint8_t *val,
			 int len)
{
...
int LowLevel::r82xx_write(struct r82xx_priv *priv, uint8_t reg, const uint8_t *val,
		       unsigned int len)
{
	int rc;
	...
	rc = SendI2cbytes(priv->cfg->i2c_addr, reg, val, len);
...
int LowLevel::r82xx_write_reg(struct r82xx_priv *priv, uint8_t reg, uint8_t val)
{
	return r82xx_write(priv, reg, &val, 1);
}

int LowLevel::r82xx_read_cache_reg(struct r82xx_priv *priv, int reg)
{
...
int LowLevel::r82xx_write_reg_mask(struct r82xx_priv *priv, uint8_t reg, uint8_t val,
				uint8_t bit_mask)
{
...
uint8_t LowLevel::r82xx_bitrev(uint8_t byte)
{
...
int LowLevel::r82xx_read(struct r82xx_priv *priv, uint8_t reg, uint8_t *val, int len)
{
	int rc = RecvI2cbytes(priv->cfg->i2c_addr, reg, val, len);
...
int LowLevel::r82xx_set_mux(struct r82xx_priv *priv, uint32_t freq)
{
...
int LowLevel::r82xx_set_pll(struct r82xx_priv *priv, uint32_t freq)
{
	int rc, i;
...
int LowLevel::r82xx_sysfreq_sel(struct r82xx_priv *priv, uint32_t freq,
			     uint32_t delsys)
{
	int rc;
...
int LowLevel::r82xx_set_tv_standard(struct r82xx_priv *priv,
				 unsigned bw,
				 uint32_t delsys)
...
int LowLevel::r82xx_read_gain(struct r82xx_priv *priv)
{
	uint8_t data[4];
	int rc;
...
int LowLevel::r82xx_set_gain(struct r82xx_priv *priv, int set_manual_gain, int gain)
{
	int rc;
...
int LowLevel::r82xx_set_bandwidth(struct r82xx_priv *priv, int bw, uint32_t rate)
{
	int rc;
	unsigned int i;
	int real_bw = 0;
...
int LowLevel::r82xx_set_freq(struct r82xx_priv *priv, uint32_t freq)
{
	int rc = -1;
	...
	if (rc < 0)
	{
		fprintf(stderr, "modrzywo1 %s: failed=%d\n", __FUNCTION__, rc);
		goto err;
	}
	...
	if (rc < 0 || !priv->has_lock)
	{
		fprintf(stderr, "modrzywo2 %s: failed=%d\n", __FUNCTION__, rc);
		goto err;
	}
...
int LowLevel::r82xx_standby(struct r82xx_priv *priv)
{
	int rc;
...
int LowLevel::r82xx_xtal_check(struct r82xx_priv *priv)
{
	int rc;
	unsigned int i;
	uint8_t data[3], val;
...
int LowLevel::r82xx_init(struct r82xx_priv *priv)
{
	int rc;
	...
	priv->cfg2.i2c_addr = R820T2_ADDR;
	priv->cfg2.rafael_chip = CHIP_R820T;
	priv->cfg2.xtal = 32000000;

	priv->cfg2.max_i2c_msg_len = 255;
	priv->cfg2.use_predetect = 0;
	...
	rc = r82xx_set_tv_standard(priv, 3, 0);
	if (rc < 0)
		goto err;

	rc = r82xx_sysfreq_sel(priv, 0, SYS_DVBT);
